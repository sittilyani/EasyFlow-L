...
try {
    ...
    $sql = "
        SELECT
            lab_id, visitDate, mat_id , clientName, type_client,
            mode_drug_use, pregnancy_status, lab_officer_name, lab_notes, next_appointment,

            /*Clinical notes from laboratory*/

            COALESCE(NULLIF(lab_notes, ''), 'no previous_visit') AS lab_notes,
...
<head>
    <meta charset="UTF-8">
    <title>Laboratory Appointment Schedule</title>
...
<div class="header">
    <div class="container">
        <h1 class="display-5 fw-bold">Laboratory Appointment Schedule</h1>
        ...
        <table class="table table-hover">
            ...
            <?php else: ?>
                <?php foreach ($appointments as $app): ?>
                    <tr>
                        <td><?= htmlspecialchars($app['lab_id']) ?></td>
                        <td><?= formatDate($app['visitDate']) ?></td>
                        <td><?= htmlspecialchars($app['mat_id']) ?></td>
                        <td><?= htmlspecialchars($app['clientName']) ?></td>
                        <td><?= htmlspecialchars($app['type_client']) ?></td>
                        <td><?= htmlspecialchars($app['mode_drug_use']) ?></td>
                        <td><?= htmlspecialchars($app['pregnancy_status']) ?></td>
                        <td><?= htmlspecialchars($app['lab_notes']) ?></td>
                        <td><?= htmlspecialchars($app['lab_officer_name']) ?></td>
                        ...
                        <td>
                            <a href="update_laboratory_appointment.php?lab_id=<?= $app['lab_id'] ?>"
