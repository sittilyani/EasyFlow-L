...
$comp_date = date('Y-m-d h-m-s');
    ...
    $query = "SELECT *, DATE_FORMAT(comp_date, '%Y-%m-%d') as formatted_comp_date FROM patients WHERE p_id = ?";
    ...
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param('sssssssssssssssssssssssssssssssi', $mat_id, $mat_number, $clientName, $sname, $nickName, $nat_id, $dob, $sex, $reg_facility, $mflcode, $county, $scounty, $reg_date, $residence_scounty, $p_address, $client_phone, $mat_status, $transfer_id, $referral_type, $referring_facility, $reffering_fac_client_number, $accompanment_type, $peer_edu_name, $peer_edu_phone, $rx_supporter_name, $drugname, $dosage, $reasons, $current_status, $hcw_name, $comp_date, $userId);
    $stmt->execute();
    ...
    header("Refresh: 3; URL=../dashboard/admin_dashboard.php");
    exit();
}
?>

<!--Code to select name form tblusers and populate -->
...
<form method="post">
 ...
 <div class="grid-container">
    ...
    <label>Enrolment Status(TI or New):</label><br>
        <select name="mat_status" class="form-control">
            ...
            if ($result->num_rows > 0) {
                // Loop through the rows and display each sub county name as an option
                while ($row = $result->fetch_assoc()) {
                    $selected = ($row['enrolment_status_name'] == $currentSettings['mat_status']) ? 'selected' : ''; // Mark the selected sub county
                    echo "<option value='" . $row['enrolment_status_name'] . "' $selected>" . $row['enrolment_status_name'] . "</option>";
                }
            } else {
                // If no sub counties are found in the database, display a default option
                echo "<option value=''>No enrol status found</option>";
            }
            ?>
        </select>
    ...
    <label>Drug:</label><br>
        <select name="drugname" class="form-control">
            ...
            if ($result->num_rows > 0) {
                // Loop through the rows and display each referral_type name as an option
                while ($row = $result->fetch_assoc()) {
                    $selected = ($row['drugname'] == $currentSettings['drugname']) ? 'selected' : ''; // Mark the selected sub referral_type
    ...
    Initial Dosage: <input type="text" name="dosage" value="<?= $currentSettings['dosage'] ?>" required><br>

    Reasons for dose adjustments: <input type="text" name="reasons" value="<?= $currentSettings['reasons'] ?>"><br>
    ...
    <label>Current Status:</label><br>
       ...
       <!-- Health Care Worker: <input type="text" name="hcw_name" value="<?= $currentSettings['hcw_name'] ?>" required><br> -->
        <!-- Health Care Worker Name field automatically populated -->
                <label for="hcw_name">HealthCare Worker Name</label> <br>
                <input type="text" name="hcw_name" value="<?php echo htmlspecialchars($hcwName); ?>" class="readonly-input" readonly> <br>


        <!--hidden form input for comp_date -->
        Status Change Date: <input type="datetime-local" name="comp_date" value="<?= date('Y-m-d\TH:i') ?>" required><br>
...
<?php
    include "../includes/footer-shimo.php";
?>
</body>
</html>
